...
describe("a widget booted against a site with WidgetLocale = ru", () => {
  ...
  it("renders the panel chrome and composer in Russian", async () => {
    ...
    expect(panel.title.textContent).toBe("Чем мы могли бы вам помочь?");
    ...
    expect(panel.emoji.getAttribute("aria-label")).toBe("Вставить эмодзи");
    expect(panel.emojiPicker.getAttribute("aria-label")).toBe("Выбор эмодзи");
...
describe("a widget booted against a site with no WidgetLocale set", () => {
  ...
  it("renders the panel chrome and composer in English, unchanged", async () => {
    ...
    expect(panel.title.textContent).toBe("How can we help you?");
    ...
    expect(panel.emoji.getAttribute("aria-label")).toBe("Insert emoji");
    expect(panel.emojiPicker.getAttribute("aria-label")).toBe("Emoji picker");
